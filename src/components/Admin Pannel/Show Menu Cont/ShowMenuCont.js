...
import styles from "./ShowMenuCont.module.css"
import {AiFillCloseCircle} from "react-icons/ai"

const ShowMenuCont = ({individualResMenuData,setIndividualResMenuData}) => {
  return (
   <section className={styles.outerCont}>
    <div className={styles.innerCont}>
    <AiFillCloseCircle onClick={()=>setIndividualResMenuData([])} className={styles.closeIcon}/>
        <h1 className={styles.title}>Menu</h1>

        <div style={{border:"none"}} className={styles.form}>
{individualResMenuData.map((item,idx)=>{
